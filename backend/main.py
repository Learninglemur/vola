...
app = FastAPI(
    ...
    description="API for parsing trading data and uploading processed files to Google Cloud Storage",
    version="2.0.0",
)
...
def upload_to_gcs(file_name: str, data: io.BytesIO, bucket_name: str) -> str:
    """
    ...
    try:
        ...
        blob.make_public()
        logger.info("File uploaded successfully to GCS: %s", file_name)
...
def parse_datetime(datetime_str: str, format_type: str) -> pd.Series:
    """
    Parse datetime string into separate date and time components.
    """
    try:
...
def convert_to_numeric(value: str) -> float:
    """
    Convert a string to a numeric value, handling special characters.
    """
    try:
...
def find_header_row_and_format(df: pd.DataFrame) -> (int, str):
    """
    Identify the header row and format type in the dataframe.
    """
...
def extract_data(df: pd.DataFrame, header_row: int, format_type: str) -> pd.DataFrame:
    """
    Extract and process relevant columns starting from the header row.
    """
    if format_type == "Format 1":
...
async def parse_trades(file: UploadFile = File(...)):
    """
    Parse trading data and upload processed file to Google Cloud Storage.
    """
    ...
    try:
        ...
        output = io.BytesIO()
        parsed_data.to_csv(output, index=False)
        output.seek(0)

        # Upload CSV to Google Cloud Storage
        public_url = upload_to_gcs(f"parsed_{file.filename}", output, BUCKET_NAME)

        return {"message": "File processed successfully", "parsed_file_url": public_url, "data": parsed_data.to_dict(orient="records")}
    except Exception as e:
